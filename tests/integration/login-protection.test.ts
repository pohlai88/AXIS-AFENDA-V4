...
describe("Login Protection Integration", () => {
  ...
  describe("Failed Login Flow", () => {
    it("should track failed login attempts", async () => {
      // First attempt
      await rateLimiter.recordFailedLogin("email", testEmail);
      let result = await rateLimiter.checkLoginAttempt("email", testEmail);
      expect(result.remainingAttempts).toBe(4);

      // Second attempt
      await rateLimiter.recordFailedLogin("email", testEmail);
      result = await rateLimiter.checkLoginAttempt("email", testEmail);
    ...
    it("should require CAPTCHA after 3 failed attempts", async () => {
      ...
      const result = await rateLimiter.checkLoginAttempt("email", testEmail);
    ...
    it("should persist lockout across checks", async () => {
      ...
      const check1 = await rateLimiter.checkLoginAttempt("email", testEmail);
      const check2 = await rateLimiter.checkLoginAttempt("email", testEmail);
  ...
  describe("IP-Based Rate Limiting", () => {
    it("should track IP-based attempts separately", async () => {
      ...
      await rateLimiter.recordFailedLogin("email", testEmail);
      const emailResult = await rateLimiter.checkLoginAttempt("email", testEmail);

      // Make attempts from IP
      await rateLimiter.recordFailedLogin("ip", testIp);
      const ipResult = await rateLimiter.checkLoginAttempt("ip", testIp);
    ...
    it("should lock IP after 10 failed attempts", async () => {
      ...
      const result = await rateLimiter.checkLoginAttempt("ip", testIp);
      expect(result.allowed).toBe(false);
      expect(result.lockedUntil).toBeInstanceOf(Date);
    });
  ...
  describe("CAPTCHA Integration", () => {
    it("should verify valid CAPTCHA tokens", async () => {
      ...
      try {
        const result = await verifyCaptchaToken({ token: testToken, remoteIp: "127.0.0.1" });
  ...
  describe("Successful Login Reset", () => {
    it("should reset attempts after successful login", async () => {
      ...
      await rateLimiter.resetLoginAttempts("email", testEmail);
  ...
  describe("Sliding Window Behavior", () => {
    ...
    it("should maintain window for concurrent attempts", async () => {
      ...
      await Promise.all([
        rateLimiter.recordFailedLogin("email", testEmail),
        rateLimiter.recordFailedLogin("email", testEmail),
        rateLimiter.recordFailedLogin("email", testEmail),
      ]);
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle missing identifier gracefully", async () => {
      const result = await rateLimiter.checkLoginAttempt("email", "");
    ...
    it("should handle database errors gracefully", async () => {
      ...
      try {
        await rateLimiter.recordFailedLogin("email", "invalid@email@domain.com");
        // Should not throw
