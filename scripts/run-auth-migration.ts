...
export async function runAuthMigration() {
  ...
  try {
    ...
    // Check if users table exists and get its current structure
    const usersTableExists = await db.select({
    ...
    if (usersTableExists.length > 0) {
      ...
      for (const column of columnsToAdd) {
        if (!columnNames.includes(column.name)) {
          ...
          await db.execute(sql`ALTER TABLE users ADD COLUMN ${sql.identifier(column.name)} ${column.type} ${column.default ? `DEFAULT ${column.default}` : ''} ${column.constraint ? column.constraint : ''}`)
        }
      }
    ...
    try {
      await db.execute(sql`ALTER TABLE accounts ADD CONSTRAINT accounts_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE NO ACTION`)
      await db.execute(sql`ALTER TABLE sessions ADD CONSTRAINT sessions_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE NO ACTION`)
      await db.execute(sql`ALTER TABLE password_reset_tokens ADD CONSTRAINT password_reset_tokens_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE NO ACTION`)
      await db.execute(sql`ALTER TABLE user_activity_log ADD CONSTRAINT user_activity_log_user_id_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE ON UPDATE NO ACTION`)
