 ...
 * @domain auth
 * @layer api
 * @responsibility API route handler for /api/verify-email/send
 */
...
/**
 * Send verification email (delegates to Neon Auth).
 *
 * Body:
 * - email: string
...
export async function POST(request: NextRequest) {
  try {
    ...
    await auth.handler().POST(
      new NextRequest(new URL(routes.api.auth.internal.email.sendCode(), request.nextUrl.origin), {
        method: "POST",
        headers: {
