...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await auth.handler().POST(
      new NextRequest(new URL(routes.api.auth.internal.password.verifyResetToken(), request.nextUrl.origin), {
        method: "POST",
        headers: {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await auth.handler().POST(
      new NextRequest(new URL(routes.api.auth.internal.password.reset(), request.nextUrl.origin), {
        method: "POST",
        headers: {
          "content-type": "application/json",
          ...Object.fromEntries(request.headers),
