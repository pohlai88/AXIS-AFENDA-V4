...
export default function ForgotPasswordPage() {
  ...
  const redirectTo = useMemo(() => {
    ...
    return `${window.location.origin}${routes.public.resetPassword()}`
  }, [])

  useEffect(() => {
    if (!isSessionLoading && isAuthenticated) {
      // If already signed in, no need to reset password here.
      window.location.assign(routes.app.root())
    }
  ...
  return (
    <AuthShell title="Reset password" description="We will email you a secure reset link.">
      ...
      <div className="text-center text-sm text-muted-foreground">
        ...
        <Link href={routes.public.login()} className="text-primary hover:underline font-medium">
          Sign in
        </Link>
      </div>
    </AuthShell>
