...
export default function VerifyEmailPage() {
  ...
  const next = useMemo(() => {
    ...
    return param && param.startsWith("/") && !param.startsWith("//") ? param : routes.app.root()
  }, [searchParams])

  useEffect(() => {
    try {
      const last = localStorage.getItem("afenda.lastRegisteredEmail") ?? ""
  ...
  return (
    <AuthShell title="Verify your email" description="Check your inbox for a verification email.">
      ...
      <div className="flex flex-col gap-2">
        <Button variant="outline" className="w-full" onClick={() => router.push(routes.public.login())}>
          Go to sign in
        </Button>
      </div>
    </AuthShell>
